...
router.post('/p', function(req, res){
  ...
  res.send(peopleJSON);
    ...
    router.post('/login',function(req, res, next) {
      passport.authenticate('local', function(err, user, info) {
	    ...
	    req.logIn(user, function(err) {
	      ...
	      return  res.send(JSON.stringify({ status: "success",userId:user.id,
	      	       username:user.name,token: 'JWT '+token}));
	    });
	  })(req, res, next);
	});
                       //isAuthenticated
	...
	router.post('/eloqualogin',passport.authenticate('jwt', { session: false }),salesforce.login);
